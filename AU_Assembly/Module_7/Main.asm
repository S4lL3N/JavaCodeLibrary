...
.data
	myString BYTE "CPSC 3303 Programming Assignment 4", 0, 
...
main endp
	...
	; --------------------------------------------
	Exercise1 PROC
	    push edx                           ; Push used registers onto the stack
	    push ecx
	    push ax
	    push edi

	    mov   edx,OFFSET sentencePrompt    ; Prompt user for input
	    call  WriteString
	    mov   edx,OFFSET buffer1           ; Get the input
	    mov   ecx,SIZEOF buffer1
	    call  ReadString
	    mov   bufferSize1,al
  
	    mov   edx, OFFSET deleteNumPrompt ; Prompt user for number of chars to delete
	    call  WriteString
	    call  ReadHex                     ; Get the hex input
	    mov   numChars,al
  
	    mov   edx, OFFSET deletePosPrompt ; Prompt user for starting position 
	    call  WriteString
	    call  ReadHex                     ; Get the hex input
	    mov   position,al
  
	    mov   edi,OFFSET buffer1          ; Delete ecx characters from a string esi starting from position edx
	    mov   bl,numChars
	    mov   dl,position
	    mov   cl,bufferSize1
	    call  DeleteFromString
  
	    mov   edx,OFFSET buffer1          ; Write string to output
	    call  WriteString
	...
	Exercise2 PROC
	    ...
	    mov   edx,OFFSET sentencePromptS1  ; Prompt the user for S_1
	    call  WriteString
	    mov   edx,OFFSET buffer1           ; Get the input
	    mov   ecx,SIZEOF buffer1
	    call  ReadString
	    ...
	    mov   edx,OFFSET sentencePromptS2  ; Prompt the user for S_2
	    call  WriteString
	    mov   edx,OFFSET buffer2           ; Get the input
	    mov   ecx,SIZEOF buffer2
	    call  ReadString
	    mov   bufferSize2,al
  
	    mov   edx, OFFSET positionPrompt   ; Prompt user for starting position 
	    call  WriteString
	    call  ReadHex                      ; Get the hex input
	    mov   position,al
